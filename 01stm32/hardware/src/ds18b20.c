...
//	if(temp1<0)
//	{
//		sprintf(data,"-%2.1f",temp1/10.0);
////		OLED_Show_Str(53,4,date,16);
//		OLED_ShowString(6,4,data,16);
//	}
//	else
//	{
//	    sprintf(data," %2.1f",temp1/10.0);
...
float DS18B20_GetTemperature(void)  // 更合适的函数名，表示获取温度
{
    uint16_t temp1;
    char data[20];

    
    temp1 = DS18B20_Get_Temp();  // 获取原始温度数据（如 235 表示 23.5°C）
    temperature1 = temp1 / 10.0f;  // 转换为浮点数（23.5）
	  OLED_ShowString(0, 6,"Temp:", 16);
	  
    // 显示温度到 OLED（可选，如果仍需显示）
    if (temp1 < 0) 
    {
        sprintf(data, "-%2.1f", temperature1);
        OLED_ShowString(50,6,data,16);
    }
    else 
    {
        sprintf(data, " %2.1f", temperature1);
    ...
    DelayXms(20);  // 延时（可选，如果仍需延时）
    
    return temperature1;  // 返回温度值（如 23.5）
}