...
#define PROID			"l2jlWwNUn4"

#define ACCESS_KEY		"NmpreUNoRTc3QVRQY0x3S3pZNld6VTZVZHE0bm44RHM="

#define DEVICE_NAME		"d2"

extern uint16_t TDS;extern uint16_t Volume_Shiwu;extern u8 temperature;
char devid[16];
...
uint16_t OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"TDS\":{\"value\":%d},",TDS);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"temperature\":{\"value\":%d},",temperature);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"volume\":{\"value\":%d}",Volume_Shiwu);
	strcat(buf, text);	
	strcat(buf, "}}");
